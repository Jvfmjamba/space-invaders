...
typedef struct Heroi{//update franca
    Rectangle pos;
    Color color;
    Bala bala;
    int velocidade;
...
//update frança
    ...
    /*direcao = 1 faz heroi mover para direita, direcao = 0 faz mover para esquerda*/
    j->heroi.direcao = 1;
...
void AtiraBalas(Jogo *j){
    ...
    if (IsKeyPressed(KEY_SPACE) && j->heroi.bala.ativa == 0) { // Detecta se a tecla de espaço foi pressionada
    ...
    } else if (ColisaoBalas(j)) {  // Colisão de bala
        j->heroi.bala.ativa = 0;
    }

    // Verifica e move a bala do herói
    if (j->heroi.bala.ativa == 1) {
        j->heroi.bala.pos.y -= j->heroi.bala.velocidade;  // Move a bala do herói para cima
        DesenhaBalas(j);  // Desenha a bala
    }
